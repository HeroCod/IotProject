...
export function updateSensorCard(deviceId, data, timestamp, type) {
    ...
    fetchDeviceOverride(deviceId).then(override => {
        ...
        // Check if card already has content (i.e., it's an update, not creation)
        const isUpdate = card.innerHTML.trim() !== '';

        if (!isUpdate) {
            // Create full card for new cards
            card.innerHTML = `
            ...
            // Update existing card - only change data parts, preserve controls
            const header = card.querySelector('h3');
            if (header) {
                header.innerHTML = `🏠 ${location.charAt(0).toUpperCase() + location.slice(1)} <span class="led-status ${ledStatus}"></span> ${manualMode}`;
            }

            const statusDiv = card.querySelector('div[style*="display: flex; align-items: center; gap: 8px;"]');
            if (statusDiv) {
                const dot = statusDiv.querySelector('div[style*="width: 10px; height: 10px"]');
                const span = statusDiv.querySelector('span[style*="font-size: 12px"]');
                if (dot) dot.style.backgroundColor = isOnline ? '#27ae60' : '#e74c3c';
                if (span) {
            ...
            const modeIndicators = card.querySelector('div[style*="display: flex; gap: 8px; margin-bottom: 15px"]');
            if (modeIndicators) {
                modeIndicators.innerHTML = `
                    <div style="padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: bold; ${override?.active ? 'background: #fff3cd; color: #856404;' : 'background: #d4edda; color: #155724;'}">
                        ${override?.active ? '🔧 Manual Mode' : '🤖 Auto Mode'}
                    </div>
                    ${data.energy_saving_mode ? '<div style="padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: bold; background: #d1ecf1; color: #0c5460;">💚 Energy Saving</div>' : ''}
            ...
            const sensorData = card.querySelector('.sensor-data');
            if (sensorData) {
            ...
            const overrideDiv = card.querySelector('.override-info');
            if (overrideDiv) {
                overrideDiv.innerHTML = overrideInfo.trim() || '';
            } else if (overrideInfo.trim()) {
                // Insert override info if not present
                const sensorDataDiv = card.querySelector('.sensor-data');
                if (sensorDataDiv) {
                    const overrideElement = document.createElement('div');
                    overrideElement.className = 'override-info';
                    overrideElement.innerHTML = overrideInfo.trim();
                    sensorDataDiv.insertAdjacentElement('afterend', overrideElement);
                }
            }

            // Update button disabled states based on online status
            const buttons = card.querySelectorAll('button');
            buttons.forEach(button => {
                if (button.onclick && button.onclick.toString().includes('sendLedCommand') ||
                    button.onclick && button.onclick.toString().includes('sendHeatingCommand') ||
                    button.onclick && button.onclick.toString().includes('sendOverrideCommand') ||
                    button.onclick && button.onclick.toString().includes('removeOverride')) {
            ...
            if (slider) {
                slider.disabled = !isOnline;
            }

            const timestampDiv = card.querySelector('.timestamp');
            if (timestampDiv) {
                timestampDiv.textContent = `🕒 Last update: ${new Date().toLocaleTimeString()}`;
            }
        }
    });
}
