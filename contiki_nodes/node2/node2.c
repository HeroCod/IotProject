...
#define CLIENT_ID "node2"
#define MQTT_PUB_TOPIC "sensors/node2/data"
...
static int temperatureCelsius = 18;         // Temperature reading in Celsius
...
static void initialize_default_schedule() {
    ...
    // Kitchen-specific schedule: breakfast (7am), lunch (12pm), dinner (7pm)
    for (int day = 0; day < 7; day++) {
        int dayOffset = day * 24;
        weeklySchedule[dayOffset + 7] = 22.0f;   // 7am - breakfast, warm kitchen
        weeklySchedule[dayOffset + 9] = 18.0f;   // 9am - reduce heating
        weeklySchedule[dayOffset + 12] = 22.0f;  // 12pm - lunch, warm kitchen
        weeklySchedule[dayOffset + 14] = 18.0f;  // 2pm - reduce heating
        weeklySchedule[dayOffset + 19] = 22.0f;  // 7pm - dinner, warm kitchen
    ...
    LOG_INFO("Default weekly schedule initialized (kitchen)\n");
...
static void handle_sensor_event()
{
    ...
    if(NETSTACK_ROUTING.node_is_reachable()) {
        ...
        LOG_INFO("NODE2_LOG: Day %d, Hour %d.%d, Occupied: %s\n",
...
PROCESS_THREAD(node2_process, ev, data) {
    ...
    LOG_INFO("  ║    ╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝╚══════╝    ║\n");
    LOG_INFO("  ║                                                   ║\n");
    LOG_INFO("  ║     KITCHEN TEMPERATURE CONTROL  NODE           ║\n");
    ...
    /* ---- Init MQTT ---- */
    mqtt_register(&mqttConnection, &node2_process, CLIENT_ID, mqtt_event, MAX_TCP_SEGMENT_SIZE);
    mqtt_connect(&mqttConnection, MQTT_BROKER_IP_ADDR, MQTT_BROKER_PORT, PUBLISH_INTERVAL, MQTT_CLEAN_SESSION_ON);
