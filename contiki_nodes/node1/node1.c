/*
 * Node 1 - Living Room Temperature Control Node
...
#define CLIENT_ID "node1"
#define MQTT_PUB_TOPIC "sensors/node1/data"
...
static int temperatureCelsius = 22;         // Temperature reading in Celsius
...
static void handle_sensor_event()
{
    ...
    if(NETSTACK_ROUTING.node_is_reachable()) {
        ...
        LOG_INFO("NODE1_LOG: Day %d, Hour %d.%d, Occupied: %s\n",
...
PROCESS_THREAD(node1_process, ev, data) {
    ...
    LOG_INFO("  ║    ╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝ ╚═╝        ║\n");
    LOG_INFO("  ║                                                   ║\n");
    LOG_INFO("  ║    LIVING ROOM TEMPERATURE CONTROL NODE           ║\n");
    ...
    /* ---- Init MQTT ---- */
    mqtt_register(&mqttConnection, &node1_process, CLIENT_ID, mqtt_event, MAX_TCP_SEGMENT_SIZE);
    mqtt_connect(&mqttConnection, MQTT_BROKER_IP_ADDR, MQTT_BROKER_PORT, PUBLISH_INTERVAL, MQTT_CLEAN_SESSION_ON);
